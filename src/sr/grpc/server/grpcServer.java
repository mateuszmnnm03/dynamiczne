...
public class grpcServer 
{
	...
	private void start() throws IOException 
	{
		...
		server = ServerBuilder.forPort(port) // tworzy serwer grpc nasluchujacy na porcie
				.executor((Executors.newFixedThreadPool(16)))
        //server = NettyServerBuilder.forAddress(socket).executor(Executors.newFixedThreadPool(16))
				.addService(new CalculatorImpl()) // rejestracja serwisu rpc
				//.addService(new CalculatorImpl())
				//.addService(new AdvancedCalculatorImpl())
				.addService(new StreamTesterImpl())
				.addService(ProtoReflectionService.newInstance()) // wlacza reflection, klient moze zapytac o moduly i pobrac strukture danych
				.build()
				.start();
